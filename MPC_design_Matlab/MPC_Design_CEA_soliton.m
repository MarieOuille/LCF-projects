...
% ABCD matrix model of the Kerr lens - 1 gas 1 bulk material - 2 bulk plates
...
input_type='Exp'; %Gauss or Exp or TwoPulses
File='Speck.dat';
...
pressure_1=1;
mat2='YAG';
pressure_2=1; %N/A if bulk

% MPC geometry
f1=0.15; %focale de la lentille=R/2
n_roundtrip=10;
...
switch mat2
    ...
    case 'ZnSe'
    n2_2=12e-19;            % ZnSe
    n_2 = sqrt(1-0.689818+4.855169./(1-0.056359./x.^2)+0.673922./(1-0.056336./x.^2)+2.481890./(1-2222.114./x.^2));%ZnSe
    tau1_2=1e-15; %ZnSe
    tau2_2=1e-15; %ZnSe
    fr_2=0;    % ZnSe
    ...
    case 'KGW Nm'
    n2_2=15e-20;          % KGW Nm
    n_2 = 1.04111 + 0.95487./(1-(140.9584./lnano).^2) - 0.5e-10.*lnano.^2 ; % KGW Nm Raman shift 901cm-1 width 7.8 cm-1 gain 3.3 cm/GW
    tau1_2=1/901e2/c0/2/pi; %Nm KGW
    tau2_2=2e-12;           %Nm KGW
    fr_2=0.35;    % Nm KGW

    case 'KGW Ng'
    n2_2=20e-20;         %Ng
    n_2 = 1.28065 + 0.75436./(1-(159.499./lnano).^2) - 1.94e-10.*lnano.^2 ; %Ng Raman shift 768cm-1 width 5.9 cm-1 gain 4.4 cm/GW
    tau1=1/768e2/c0/2/pi; %Ng
    tau2=3e-12;           %Ng
    fr=0;                   %pas encore fitté pour Ng
    
    case 'air'
    n2_2=pressure_2*3.6e-23;%air
    n_2=1+pressure_2*(0.05792105./(238.0185-lmicron.^(-2))+0.001679176./(57.362-lmicron.^(-2))); %air
    tau1_2=62.5e-15; %N2
    tau2_2=120e-15;  %N2        
    fr_2=0.75;    %fr_1=0.75 for N2
    
    case 'N2'
    n2_2=pressure_2*4e-23; %N2 global
    n_2=1+pressure_2*(6.8552e-5+3.243157e-2./(144-lmicron.^-2)); %N2
    tau1_2=62.5e-15; %N2
    tau2_2=120e-15;  %N2        
    fr_2=0.75;    %fr_1=0.75 for N2

    case 'Ar'
    n2_2=pressure_2*0.975e-23; %Ar
    n_2=1+pressure_2*(2.50141e-3./(91.012-lmicron.^-2)+5.00283e-4./(87.892-lmicron.^-2)+5.22343e-2./(214.02-lmicron.^-2)); %Ar
    tau1_2=1e-15;
    tau2_2=1e-15;         
    fr_2=0;    

    case 'Xe'
    n2_2=pressure_2*5.2e-23; %Xe
    n_2=1+pressure_2*(0.00322869./(46.301-lmicron.^-2)+0.00355393./(59.578-lmicron.^-2)+0.0606764./(112.74-lmicron.^-2)); %Xe
    tau1_2=1e-15;
    tau2_2=1e-15;         
    fr_2=0;
    
    case 'Ne'
    n2_2=pressure_2*8.7e-25; %Ne
    n_2=1+pressure_2*(0.00128145./(184.661-lmicron.^-2)+0.0220486./(376.840-lmicron.^-2)); %Ne
    tau1_2=1e-15;
    tau2_2=1e-15;         
    fr_2=0;  
end
...
dz_MPC_1=z_mat2_start1/round(z_mat2_start1/dz_MPC_mat1);
dz_MPC_1(isnan(dz_MPC_1))=dz_MPC_mat1;
dz_MPC_2=(z_mat2_stop1-z_mat2_start1)/round((z_mat2_stop1-z_mat2_start1)/dz_MPC_mat2);
dz_MPC_2(isnan(dz_MPC_2))=dz_MPC_mat2;
dz_MPC_3=(z_mat2_start2-z_mat2_stop1)/round((z_mat2_start2-z_mat2_stop1)/dz_MPC_mat1);
dz_MPC_3(isnan(dz_MPC_3))=dz_MPC_mat1;
dz_MPC_4=(z_mat2_stop2-z_mat2_start2)/round((z_mat2_stop2-z_mat2_start2)/dz_MPC_mat2);
dz_MPC_4(isnan(dz_MPC_4))=dz_MPC_mat2;
dz_MPC_5=(Lres-z_mat2_stop2)/round((Lres-z_mat2_stop2)/dz_MPC_mat1);
dz_MPC_5(isnan(dz_MPC_5))=dz_MPC_mat1;


z_MPC1=dz_MPC_1:dz_MPC_1:z_mat2_start1;
z_MPC2=z_mat2_start1+dz_MPC_2:dz_MPC_2:z_mat2_stop1;
z_MPC3=z_mat2_stop1+dz_MPC_3:dz_MPC_3:z_mat2_start2;
z_MPC4=z_mat2_start2+dz_MPC_4:dz_MPC_4:z_mat2_stop2;
z_MPC5=z_mat2_stop2+dz_MPC_5:dz_MPC_5:Lres;

z_MPC=[z_MPC1 z_MPC2 z_MPC3 z_MPC4 z_MPC5];
...
M_rt=[1 0;-1/f1 1]*[1 z_mat2_start1/n_1(nt/2);0 1]*[1 (z_mat2_stop1-z_mat2_start1)/n_2(nt/2);0 1]*[1 (z_mat2_start2-z_mat2_stop1)/n_1(nt/2);0 1]*[1 (z_mat2_stop2-z_mat2_start2)/n_2(nt/2);0 1]*[1 (Lres-z_mat2_stop2)/n_1(nt/2);0 1]*[1 0;-1/f1 1]*[1 (Lres-z_mat2_stop2)/n_1(nt/2);0 1]*[1 (z_mat2_stop2-z_mat2_start2)/n_2(nt/2);0 1]*[1 (z_mat2_start2-z_mat2_stop1)/n_1(nt/2);0 1]*[1 (z_mat2_stop1-z_mat2_start1)/n_2(nt/2);0 1]*[1 z_mat2_start1/n_1(nt/2);0 1];
